...
#define kEmptyWord				"<--->"
	...
	BOOL			HWR_RecognizerAddStroke( RECOGNIZER_PTR pRecognizer, CGPoint * pStroke, int nStrokeCnt );
	BOOL			HWR_Recognize( RECOGNIZER_PTR pRecognizer );
	BOOL			HWR_Reset( RECOGNIZER_PTR pRecognizer );
	const UCHR *	HWR_RecognizeInkData( RECOGNIZER_PTR pRecognizer, INK_DATA_PTR pInkData, int nDataLen, BOOL bAsync, BOOL bFlipY, BOOL bSort, BOOL bSelOnly );
