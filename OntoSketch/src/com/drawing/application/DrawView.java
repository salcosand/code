			...
			for (int i = 0; i < drawingObjects.getPathList().getPaths().size(); i++) 
			{
				...
				if (path.isHighlighted()) 
				{
					...
					if (nStrokeCnt > 0 && ((component instanceof DrawingLeaf))) 
					{
						...
						int index = WritePadAPI.recoGetStrokeByPoint(path.getOriginalVertices()[0].x, path.getOriginalVertices()[0].y);
	...
	private final Handler mHandler = new Handler() 
	{
		...
		public void handleMessage(Message msg) 
		{
			...
			if (mCurrMessage != null && changeAlternatives) 
			{
				...
				{
					alternativeWordListView.getChildAt(i).setVisibility(Button.GONE);
				}
					...
					if (key != "result") 
					{
						...
						if (btn != null) 
						{
							...
							if (gestureRecognitionEnabled)
								if (key.length() > 1 && (!key.contains("O") && !key.contains("o") && !key.contains("0"))) 
								{
				...
				ImageButton	btn2 = (ImageButton) alternativeWordListView.getChildAt(6);
				btn2.setVisibility(ImageButton.VISIBLE);
			}
